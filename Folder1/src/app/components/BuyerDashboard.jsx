...
export function BuyerDashboard() {
  ...
  useEffect(() => {
    // Load products from localStorage (shared with farmer dashboard)
    const savedProducts = localStorage.getItem('products');
    if (savedProducts) {
      const parsed = JSON.parse(savedProducts);
      const updatedProducts = parsed.map(p => {
        if (!p.image) {
          if (p.id === '1') return { ...p, image: '/images/tomatoes.png' };
          if (p.id === '2') return { ...p, image: '/images/milk.png' };
          if (p.id === '3') return { ...p, image: '/images/honey.png' };
          if (p.id === '4') return { ...p, image: '/images/eggs.png' };
        }
        return p;
      });
      setProducts(updatedProducts);
      // Update localStorage if images were added
      if (JSON.stringify(parsed) !== JSON.stringify(updatedProducts)) {
      ...
      const mockProducts = [
        ...
        },
        {
          ...
          name: 'Honey',
          category: 'Other',
          description: 'Natural honey from mountain flowers',
          price: 350,
          unit: 'kg',
          stock: 45,
          minStock: 10,
