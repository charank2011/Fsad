...
import {
  ...
  Package } from
'lucide-react';
import { getProducts, updateProduct } from '../data/mockData';
...
export function InventoryManagement() {
  ...
  const handleStockUpdate = (id, newStock) => {
    ...
    const updated = updateProduct(id, { stock: newStock });
    if (updated) {
  ...
  sort((a, b) => {
    const aPercentage = a.stock / a.lowStockThreshold * 100;
    const bPercentage = b.stock / b.lowStockThreshold * 100;
  ...
  const stats = {
    ...
    critical: products.filter((p) => {
      const percentage = p.stock / p.lowStockThreshold * 100;
      return p.stock === 0 || percentage <= 50;
    }).length,
    low: products.filter((p) => {
      const percentage = p.stock / p.lowStockThreshold * 100;
      return percentage > 50 && percentage <= 100;
    }).length,
    good: products.filter((p) => {
      const percentage = p.stock / p.lowStockThreshold * 100;
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            filteredProducts.map((product) => {
              const status = getStockStatus(product);
              const percentage = Math.min(product.stock / product.lowStockThreshold * 100, 100);
                      ...
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            {product.stock} {product.unit} in stock • Threshold: {product.lowStockThreshold} {product.unit}
                          </p>
                        </div>
                      </div>
